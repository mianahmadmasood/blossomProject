        ...
        <div class="kt-portlet__body">
            ...
            <table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_1">
                ...
                <tbody class="row_position">
                    ...
                    <tr id="{{$banner->id}}" data-value="top_brands">
                        @php
                        ...
                        <td>
                            @if($banner->archive == 0)
                            <a href="{{route('updateArchiveBanner', ['uid' => $banner->uuid,'status' => 'in-active','type' => 'top_brands'])}}" class="alterConfirmMassageForbanner">
                                <i class="fa fa-trash">Delete</i>
                            </a>
                            @else
                            <a href="{{route('updateArchiveBanner', ['uid' => $banner->uuid,'status' => 'active','type' => 'top_brands'])}}" class="alterConfirmMassageForbannerActive">
                                <i class="fa fa-trash">Unarchive</i>
                            </a>
                            @endif
                            <br>
                                @if($banner->status == 0)
                            <a href="{{route('updateStatusBanner', ['uid' => $banner->uuid,'status' => 'in-active','type' => 'top_brands'])}}" >
                                <i class="la la-trash">Deactive</i>
                            </a>
                            @else
                            <a href="{{route('updateStatusBanner', ['uid' => $banner->uuid,'status' => 'active','type' => 'top_brands'])}}" >
