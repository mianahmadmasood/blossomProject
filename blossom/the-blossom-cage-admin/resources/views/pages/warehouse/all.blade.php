            ...
            <div class="kt-portlet__head-toolbar">
                <div class="kt-portlet__head-wrapper">
                    <div class="kt-portlet__head-actions">
                        &nbsp;
                        <a href="{{route('createWarehouse')}}" class="btn btn-brand btn-elevate btn-icon-sm">
            ...
            <div class="kt-form kt-form--fit kt-margin-b-20">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        &nbsp;&nbsp;
                        <a href="{{route('getWarehouses')}}" class="btn btn-secondary btn-secondary--icon" id="kt_reset">
                            <span>
...
{{--                        <th>Products List</th>--}}
                ...
                <tbody>
                    ...
                    <tr>
                        <td> {{$key+1}}</td>
                        <td> {{$warehouse->en_title}}</td>
                        <td> {{$warehouse->ar_title}}</td>
                        @php 
                        ...
                        @endphp
                        @if(!empty($warehouse->image))
                        <td> <img class="imagSize" src="{{config('paths.home_url') . 'thumbnails/large/warehouses/' . $image}}"></td>
                         @else
                        <td> <img class="imagSize" src="{{config('paths.home_url') . 'thumbnails/large/categories/' . $image}}"></td>
                       @endif
                            <td>
                            <a href="{{route('editWarehouse', ['uid' => $warehouse->uuid])}}">
                                <i class="la la-edit">Edit</i>
                            </a>&nbsp;  &nbsp;
                            @if($warehouse->archive == 0)
                            <a href="{{route('updateStatusWarehouse', ['uid' => $warehouse->uuid,'status' => 'in-active'])}}" class="alterConfirmMassageForWarehouse">
                                <i class="fa fa-trash">Delete</i>
                            </a>
                            @else
                            <a href="{{route('updateStatusWarehouse', ['uid' => $warehouse->uuid,'status' => 'active'])}}" class="alterConfirmMassageForWarehouseActive">
