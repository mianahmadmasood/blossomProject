            ...
            <div class="kt-portlet__head kt-portlet__head--lg">
                ...
                <span class="kt-portlet__head-icon">
                    ...
                    <h3 class="kt-portlet__head-title">
                        Flash Deals For Home Page
                    </h3>
                </div>
            </div>
            @include('partials.falshDeals')
                    ...
                    <table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_1">
                        ...
                        @foreach($banners as $key => $banner)
                            ...
                            @if(!empty($banner['items']) && !empty($banner['category'])  && !empty($banner['sub_category']) )
                                <tr id="{{$banner->id}}" data-value="falshDeals">
                                    ...
                                    <td>
                                        @if($banner->archive == 0)
                                            <a href="{{route('updateArchiveBanner', ['uid' => $banner->uuid,'status' => 'in-active','type' => 'falshDeals'])}}"
                                               class="alterConfirmMassageForTopSaleItem">
                                                <i class="fa fa-trash">Delete</i>
                                            </a>
                                        @else
                                            <a href="{{route('updateArchiveBanner', ['uid' => $banner->uuid,'status' => 'active','type' => 'falshDeals'])}}"
                                        ...
                                        <br>
                                        @if($banner->status == 0)
                                            <a href="{{route('updateStatusBanner', ['uid' => $banner->uuid,'status' => 'in-active','type' => 'falshDeals'])}}">
                                                <i class="la la-trash">Deactive</i>
                                            </a>
                                        @else
                                            <a href="{{route('updateStatusBanner', ['uid' => $banner->uuid,'status' => 'active','type' => 'falshDeals'])}}">
